...
for idx in range(len(title_list)):
    ...
    hits = 0.0
    temp_true_apis=[]
    for x in list(true_apis):
        class_api=".".join(x.split(".")[:-1])
        temp_true_apis.append(class_api)
        
    len_api=len(temp_true_apis)
    
    for i,api in enumerate(recommended_api):
        class_api=".".join(api.split(".")[:-1])
        
        if api in true_apis:
            Bi_Encoder_hit_list[i]=1
            
        if class_api in temp_true_apis and pos == -1:
            pos = i+1
        if class_api in temp_true_apis:
            hits += 1
            Bi_Encoder_hit_recall_list[i]=1
            tmp_map += hits/(i+1)
            temp_true_apis.remove(class_api)
...
df.to_csv("result/BIKER_original_test_result_class_level.csv")
