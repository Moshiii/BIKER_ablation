...
def preprocess_pythondoc(javadoc,javadoc_dict_classes,javadoc_dict_methods,idf,w2v):

    for api in javadoc:
        # package_name,method_name,docstring
        #package_name, class_name, class_description
        #javadoc_dict_classes[api.class_name] = api.package_name+'.'+api.class_name

        description_words = [SnowballStemmer('english').stem(word) for word in api.class_description]

        api.methods_matrix.append(similarity.init_doc_matrix(description_words,w2v))
        api.methods_idf_vector.append(similarity.init_doc_idf_vector(description_words,idf))
        
        javadoc_dict_methods[api.package_name+'.'+api.class_name] = api.package_name+'.'+api.class_name
...
def recommend_python_api_processed(query_matrix,query_idf_vector,top_questions,questions,javadoc,javadoc_dict_methods,topk):
    # remember that top_questions is a dictionary of the top-k most relevant questions of the query
    # the key is question id, the value is the similarity between the question and the query
    # questions is a list including all questions (api related) in StackOverflow
    # javadoc is a list including all api classes
    ...
    for api in javadoc:
        ...
        method_name = api.package_name+'.'+api.class_name
        
        if method_name not in api_methods:
            continue
            
        else:
            doc_sim = similarity.sim_doc_pair(query_matrix,api.methods_matrix[0],query_idf_vector,api.methods_idf_vector[0])
            so_sim = api_methods[method_name]
